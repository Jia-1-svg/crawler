...
func ConsulInit() error {
	...
	consulConfig.Address = fmt.Sprintf("%s:%d", config.Config.Consul.Host, config.Config.Consul.Port)
	var err error
	...
	serviceID = fmt.Sprintf("%s-%d", config.Config.Consul.ServiceName, time.Now().Unix())
	checkID := fmt.Sprintf("%s-health", serviceID)
	registration := &api.AgentServiceRegistration{
		ID:      serviceID,
		Name:    config.Config.Consul.ServiceName,
		Address: "localhost",
		Port:    config.Config.Consul.ServicePort,
		Checks: []*api.AgentServiceCheck{
			{
				CheckID:                        checkID,
				Name:                           "TTL Health Check",
				TTL:                            fmt.Sprintf("%ds", config.Config.Consul.TTL),
	...
	go func() {
		ticker := time.NewTicker(time.Duration(config.Config.Consul.TTL/2) * time.Second)
		defer ticker.Stop()
		for range ticker.C {
